...
class MarketingMaterial(Document):
    ...
    def fix_attachment(self):
        # ~ Making sure that the attachment_image gets attach when is a new doc
        file_doc = frappe.get_all("File", 
            filters=[
                ["attached_to_doctype", "=", "Marketing Material"], 
                ["file_url", "=", self.image], 
                ["attached_to_name", "LIKE", "New Marketing Material%"]
            ])
        if len(file_doc) > 0:
    ...
    def push_notifications(self, reason=None, item=None, season=None):
        reason_sql = None
        if reason == "Item":
